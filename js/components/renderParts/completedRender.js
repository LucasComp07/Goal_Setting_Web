...
function completedRender(data) {
    ...
    const node = data.map((todo, num) => {
            ...
            const checkCompleted = arrayCompleted.every(e => e === true);
    ...
    const cleanNode = node.filter(item => item !== undefined);

    bodychild1.append(...cleanNode);
}
